"""Abuse controls: hourly rate limiting, a race-safe daily spend cap, and
(optional) Cloudflare Turnstile bot verification. All three are env-gated so
local/demo mode never blocks on an unset secret."""
...
def reserve_spend(db: Session, estimated_usd: float | None = None) -> dict:
    ...
    amount = estimated_usd if estimated_usd is not None else settings().estimated_cost_per_invoice_usd
...
def refund_spend(db: Session, amount: float) -> None:
    if amount <= 0:
        return
    today = _today()
    row = db.get(SpendLedger, today)
